...
export async function getSalesforceConnect(req, res) {
...
export async function saveSalesforceConnect(req, res) {
    const tenantId = parseTenantId(req, res);
    if (tenantId === null)
        return;
    const { salesforceLoginUrl, salesforceApiUrl, salesforceToken, refreshToken, instanceUrl, signature, clientId, clientSecret, username, password, grantType, } = req.body ?? {};
    try {
...
export async function deleteSalesforceConnect(req, res) {
    const tenantId = parseTenantId(req, res);
    if (tenantId === null)
        return;
    try {
...
export async function getWhatsappConnect(req, res) {
    const tenantId = parseTenantId(req, res);
    if (tenantId === null)
        return;
    try {
...
export async function saveWhatsappConnect(req, res) {
    const tenantId = parseTenantId(req, res);
    if (tenantId === null)
        return;
    const { whatsappBusinessNumber, businessAccountId, apiVersion, accessToken, encryptedToken, } = req.body ?? {};
    try {
        const [existing] = await db
            .select()
            .from(whatsappConnect)
...
export async function deleteWhatsappConnect(req, res) {
    const tenantId = parseTenantId(req, res);
    if (tenantId === null)
        return;
    try {
